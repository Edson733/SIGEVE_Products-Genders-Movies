...
const findAll = async () => {
    const sql = `SELECT sales_tickets.id_ste, sales_tickets.movie_show_ste, clients.name_cet, 
                    sales_tickets.total_count, sales_tickets.status_ste FROM sales_tickets 
                    JOIN movie_shows JOIN clients ON sales_tickets.movie_show_ste = movie_shows.id_msw 
                    && sales_tickets.client_spo = clients.id_cet;`;
...
const findEnable = async () => {
    const sql = `SELECT sales_tickets.id_ste, sales_tickets.movie_show_ste, clients.name_cet, 
                    sales_tickets.total_count, sales_tickets.status_ste FROM sales_tickets 
                    JOIN movie_shows JOIN clients ON sales_tickets.movie_show_ste = movie_shows.id_msw 
                    && sales_tickets.client_spo = clients.id_cet WHERE status_ste = 1;`;
    return await query(sql, []);
};
...
const findById = async (id) => {
    ...
    const sql = `SELECT sales_tickets.id_ste, sales_tickets.movie_show_ste, clients.name_cet, 
                    sales_tickets.total_count, sales_tickets.status_ste FROM sales_tickets 
                    JOIN movie_shows JOIN clients ON sales_tickets.movie_show_ste = movie_shows.id_msw 
                    && sales_tickets.client_spo = clients.id_cet WHERE id_ste = ?;`;
