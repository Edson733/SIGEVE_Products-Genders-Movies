...
const insert = async (req, res = Response) => {
    try{
        const {name_mve, duration, gender, availability_mve} = req.body;
        const results = await save({name_mve, duration, gender, availability_mve});
        res.status(200).json({results});
    }catch (err) {
        console.log(err);
        const message = validateError(err);
...
const modific = async (req, res = Response) => {
    try{
        const {name_mve, duration, gender, availability_mve, id_mve} = req.body;
        const results = await update({name_mve, duration, gender, availability_mve, id_mve});
        res.status(200).json({results});
    }catch (err) {
        console.log(err);
        const message = validateError(err);
